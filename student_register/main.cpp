...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            std::string sn = parts.at(1);

            bool found_sn = false;

            for(auto pair: student_numbers) {
                if (pair.second->student_number == sn) {
                    found_sn = true;
                }
            }
            ...
            if (!found_sn) {
                ...
                if (is_valid_phone_number(new_pn)) {

                    for(auto pair: student_numbers) {
                        if (pair.second->student_number == sn) {
                            pair.second->phone_number = new_pn;
                         }
                    }
                    for(auto pair: user_ids) {
                        if (pair.second->student_number == sn) {
                    ...
                    std::ofstream file_write("new_file");

                    for(auto pair: user_ids) {
                        file_write << pair.second->student_number << ";" << pair.second->user_id << ";"
                        << pair.second->name << ";" << pair.second->phone_number << ";" << pair.second->email
                        << ";" << pair.second->skype << "\n";
                    }
                    ...
                    std::rename("new_file", file_name.c_str());
